...
def show_count(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        current_x = x
        current_y = y
        # gap_list = compute_rib_gap(img_src,current_y)
        gap_list = [0,19,24,10,12,11,10,10,14,33,10,15]
        gap = cv2.getTrackbarPos('gap', 'RibCounting')
        isShow = True

        for i in range(len(gap_list)):
            if(y-i*spacing_height > 0):
                cv2.putText(annotated_original_img, str(count), (x, y - gap_list[i]),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
                # cv2.putText(annotated_original_img, str(count), (x, y - i*gap-spacing_height),
                #             cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
                if(x<width/2):
                    cv2.line(annotated_original_img, (x - 30, y - gap_list[i] - 2 * spacing_height),
                             (x - 10, y - gap_list[i] - 2 * spacing_height), (0, 0, 0), 1)
                    # if(i != 11):
                    ...
                    cv2.line(annotated_original_img, (x + 20, y - gap_list[i] - 2 * spacing_height),
                             (x + 40, y - gap_list[i] - 2 * spacing_height), (0, 0, 0), 1)
                    # if(i != 11):
                ...
                print(str(count) + " height = " + str(y - gap_list[i]))
...
cv2.createTrackbar('gap', 'RibCounting', 17, 30, onTrackbarChange)
